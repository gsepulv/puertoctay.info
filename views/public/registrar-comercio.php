...
<section class="section-sm">
    <div class="container" style="max-width: 600px;">
        ...
        <?php if (!empty($_SESSION['registro_exito'])): ?>
            ...
            <div style="background: #F0FDF4; border: 2px solid #22C55E; border-radius: var(--radius-lg); padding: 2rem; text-align: center; margin-bottom: 2rem;">
                <span style="font-size: 2.5rem; display: block; margin-bottom: 0.5rem;">✅</span>
                <h2 style="margin-bottom: 0.5rem; color: #166534;">¡Registro exitoso!</h2>
                <p style="color: #166534; margin: 0;">Tu comercio será revisado por nuestro equipo. Te notificaremos por email cuando esté publicado.</p>
            </div>
        ...
        <form method="POST" action="<?= SITE_URL ?>/registrar-comercio" style="background: var(--white); border-radius: var(--radius-lg); padding: 2rem; border: 1px solid var(--border); box-shadow: var(--shadow-sm);">
            <?= csrf_field() ?>
            ...
            <div style="background: #FEF3C7; border: 2px solid #F59E0B; border-radius: var(--radius-md); padding: 1.25rem; margin-bottom: 2rem;">
                <h3 style="margin: 0 0 0.25rem; font-size: 1rem; color: #92400E; text-transform: uppercase; letter-spacing: 0.5px;">
                    Términos y Políticas — Lectura Obligatoria
                </h3>
                <p style="margin: 0 0 1rem; font-size: 0.8rem; color: #B45309;">
                    Debes aceptar todas las políticas para registrarte.
                </p>

                <?php
                $politicas = [
                    ['name' => 'politica_terminos', 'label' => 'Términos y Condiciones', 'url' => SITE_URL . '/terminos-y-condiciones'],
                    ['name' => 'politica_privacidad', 'label' => 'Política de Privacidad', 'url' => SITE_URL . '/politica-de-privacidad'],
                    ['name' => 'politica_cookies', 'label' => 'Política de Cookies', 'url' => SITE_URL . '/politica-de-cookies'],
                ];
                ...
                <div style="background: #fff; border: 1px solid #FED7AA; border-radius: 8px; padding: 0.75rem 1rem; margin-bottom: 0.5rem;">
                    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 0.5rem;">
                        <a href="<?= $pol['url'] ?>" target="_blank" style="font-weight: 600; font-size: 0.9rem; color: var(--primary); text-decoration: underline;">
                            <?= $pol['label'] ?>
                        </a>
                        <div style="display: flex; gap: 1rem;">
                            <label style="display: flex; align-items: center; gap: 0.3rem; cursor: pointer; font-size: 0.85rem; color: #15803D; font-weight: 600;">
                                <input type="radio" name="<?= $pol['name'] ?>" value="acepto" <?= ($d[$pol['name']] ?? '') === 'acepto' ? 'checked' : '' ?> required> Acepto
                            </label>
                            <label style="display: flex; align-items: center; gap: 0.3rem; cursor: pointer; font-size: 0.85rem; color: #DC2626; font-weight: 600;">
                                <input type="radio" name="<?= $pol['name'] ?>" value="rechazo" <?= ($d[$pol['name']] ?? '') === 'rechazo' ? 'checked' : '' ?>> Rechazo
                            </label>
                        </div>
                    </div>
                </div>
            ...
            <div class="form-group">
                ...
                <textarea name="descripcion_comercio" rows="4" placeholder="Describe tu negocio, qué ofreces y qué te hace especial..." minlength="20" required><?= htmlspecialchars($d['descripcion_comercio'] ?? '') ?></textarea>
